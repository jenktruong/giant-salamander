...
Just looking populations in the final data collection year, 2017, the proportions of giant salamanders were almost 50% when comparing salamander counts for all channel types across clearcut and original growth sections. This suggests salamanders had almost no preferences between the forest sections, which mirrors the visual results found in Results A. However, there is a large different in salamander counts between the different channel types. There were more giant salamanders in cascades (CC = `r mack_creek_class_count$n[1]`, OG = `r mack_creek_class_count$n[2]`) than from the numbers in pools (CC = `r mack_creek_class_count$n[3]`, OG = `r mack_creek_class_count$n[4]`) and side channels (CC = `r mack_creek_class_count$n[5]`, OG = `r mack_creek_class_count$n[6]`) combined. 
...
ggplot(data = mack_creek_weight_section, 
  ...
  facet_wrap(~section) # Facet by section
```
***Figures 4 and 5.*** *Exploring distributions of salamanders weights in different forest sections (CC/OG). Visually, neither the histogram (above) shows normal distributions of data and the non-linear QQ plots (below) support that analysis.*
...
ggplot() + # Leave ggplot blank to graph from multiple data frames
  geom_histogram(data = mack_creek_weight_section, # Start beeswarm plot
                aes(x = weight)) + 
  labs(x = "Average Salamander Weight",
       y = "Number of Salamanders",
       title = "Distribution of Giant Salamander Weights in Mack Creek, 2017") + # Add labels and title
  theme_minimal() +
...
```
***Figure 6.*** *Distribution of salamader weights in clear cut and old growth forest sections. The red dot and lines represent the mean weight and error bar. Visually, the variance of salamanders weights *
...
Therefore, the mean salamander weights in both sections are significantly different. 
...
ggplot() + # Leave ggplot blank to graph from multiple data frames
  ...
  theme_minimal()
```
***Figure 8.*** *Distribution of salamader weights in cascades, pools, and side channels. The red dot and lines represent the mean weight and error bar. Visually, the variance of salamanders weights *

After doing our exploratory analysis, we decide to do an **one-way ANOVA ** test to explore salamander weights between the three channel classifications. There are three samples to test, so we need to use ANOVA. 
...
3. The mean salamander weights from each forest section are not equal to each other. 
4. Though our results suggest that the mean salamander weights are not equal across the three channel types (channel, pool, and side channel), comparing by means may not be the best statistical measure to compare the groups. 
