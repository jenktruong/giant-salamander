...
ggplot(data = mack_creek_weight_section, 
  ...
  facet_wrap(~section) # Facet by section
```
The histograms show that the data of salamander weights for both sections are positively skewed, and the qq plots are exponential. 
...
The t-statistic is `r weight_section_ttest$statistic` and the p-value is `r weight_section_ttest$p.value`. We have sufficient evidence to reject the null hypothesis that the means in both sections are the same.
